...
# 📦 Airtable ve embedding modelini başlat
table = Table(AIRTABLE_API_KEY, AIRTABLE_BASE_ID, AIRTABLE_TABLE_NAME)
embedding_model = SentenceTransformer("sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2")


# 🧹 Soru metni temizleyici
def temizle(metin):
...
def add_record(question: str, answer: str, source: str = "manual", embedding: list = None):
    ...
    try:
        ...
        return created["id"]  # ⬅️ record_id'yi geri döndür
    except Exception as e:
        print("❌ Airtable ekleme hatası:", e)
        return None


# 🔁 usage_count değerini artır
...
# 🔍 En yakın kayıt (embedding karşılaştırmalı) bul
...
# 📥 Örnek veri setini Airtable'a yükle
...
# 🔁 İkincil cevap (secondary_answer) güncelle
def update_secondary_answer(record_id: str, secondary_answer: str):
    try:
        table.update(record_id, {"secondary_answer": secondary_answer})
        print("📝 secondary_answer güncellendi.")
    except Exception as e:
        print("❌ İkincil cevap güncelleme hatası:", e)


# 🔍 Kayıt ID’si ile tek kayıt al
def get_record_by_id(record_id: str):
    try:
        record = table.get(record_id)
        return record["fields"]
    except Exception as e:
        print("❌ Kayıt getirme hatası:", e)
        return None
